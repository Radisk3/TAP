...
<?php
	...
	if ($result = $mysqli->query($query)) {
		...
		echo '<h3> Movimentação da conta  - Depósito outra conta </h3>';
		echo '<form method="POST" action="../salvar/salva_movimento.php" autocomplete="off">';
#		echo '<input type="text" name="ID_RCA" placeholder="Índice da conta"  value = "'.$row['id_rca'].'" required readonly="true" size=6 >';
		...
		if ($result = $mysqli->query($query)) {
			...
			$row = $result->fetch_assoc();
			#var_dump($row);
			echo '<input type="text" name="ID_RCA" hidden="true" placeholder="Índice da conta"  value = "'.$row['id_rca'].'" required readonly="true" size=6 >';

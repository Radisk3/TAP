...
<?php
	...
	if ($result = $mysqli->query($query)) {
		...
		$row = $result->fetch_assoc();
			...
			echo '<div class="meio">';
			echo '<h3> Edição do banco '.$row['NOME'].' </h3>';
			echo '<form method="POST" action="../salvar/salva_banco.php" autocomplete="off">';
			...
			echo '<b>Código</b> <input type="text" name="Codigo" value = '.$row['CODIGO'].' placeholder="Codigo do banco" pattern="[0-9]+$" maxlength="10" required><br>';
			echo '<br>';
			echo '<b>Nome</b> <input type="text" name="Nome"  value = "'.$row['NOME'].'" placeholder="Nome completo do banco" maxlength="50" required><br>';
			echo '<font size="1" color="red" >Informe somente números</font><br>';
			echo '<b>CNPJ</b> <input type="text" name="CNPJ" placeholder="CNPJ do banco"  value = '.$row['CNPJ'].' pattern="[0-9]+$" maxlength="14" required><br>';
			echo '</font>';
			echo '<br>';
			echo '<button type="submit" value="Alterar">Alterar</button>';
			echo '</form>';
			echo '<br>';
